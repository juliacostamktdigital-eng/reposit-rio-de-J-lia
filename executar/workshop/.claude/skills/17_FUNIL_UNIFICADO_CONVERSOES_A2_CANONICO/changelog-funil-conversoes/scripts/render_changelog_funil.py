#!/usr/bin/env python3
"""Change log do funil A-2 (playbook 17 § Gerenciado) — Markdown consolidado e auditoria de entradas."""
...
def default_document() -> dict[str, Any]:
    return {
        "meta": {
            "projeto": "[Nome]",
            "versao_funil_atual": "v1.0",
            "dono_changelog": "",
            "link_artefato_funil": "",
            "notas_cadencia": "Quinzenal até estabilizar; mensal após (playbook 17).",
        },
...
def audit(d: dict[str, Any]) -> list[str]:
    ...
    if not filled(m.get("link_artefato_funil")):
        issues.append("meta.link_artefato_funil")
    ...
    for i, e in enumerate(d.get("historico") or [], 1):
        ...
        if not _entry_started(e):
            continue
        pr = f"historico[{i}]"
        for k in ("data", "autor", "dimensao", "resumo", "antes", "depois", "motivo", "impacto_esperado"):
        ...
        if st == "publicado" and not filled(e.get("impacto_observado")):
            issues.append(f"(aviso) {pr}: impacto_observado vazio — preencher na primeira revisão de dados")
...
def render_md(d: dict[str, Any]) -> str:
    m = d.get("meta") or {}
    parts: list[str] = []
    parts.append("# Change log — Funil unificado (A-2)\n\n")
    parts.append(
        f"- **Projeto:** {block(m.get('projeto'))}\n"
        f"- **Versão funil atual (registro):** {block(m.get('versao_funil_atual'))}\n"
        f"- **Dono do log:** {block(m.get('dono_changelog'))}\n"
        f"- **Artefato A-2:** {block(m.get('link_artefato_funil'))}\n"
    ...
    hist_sorted = sorted(
        hist,
        key=lambda x: str(x.get("data") or ""),
        reverse=True,
    )
    ...
    for e in hist_sorted:
        ...
        parts.append(
            f"**Versão funil após:** {block(e.get('versao_funil_apos'))}\n"
...
def main() -> None:
    p = argparse.ArgumentParser(description="Change log funil A-2 (playbook 17)")
