...
**Project isolation:** If `PAPERCLIP_ISOLATE_PROJECT_ISSUES=true` is set, this project is configured to restrict issue scope. You MUST append `?projectId=$PAPERCLIP_PROJECT_ID` to ALL calls to `/api/agents/me/inbox-lite` — never fetch the global inbox while this flag is set. Do not reference or act on issues from other projects.
